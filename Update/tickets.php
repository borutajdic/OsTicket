...
if($_POST && !$errors):
    ...
    if($ticket && $ticket->getId()) {
		...
		case 'add_time':
			if(!$_POST['time_spent'])
				$errors['time_spent']=__('Potreben je obračunski čas');
			if(!$_POST['time'])
				$errors['time']=__('Potreben dejanski je čas');
			...
			if(!$errors  && ($response=$ticket->addTimeCategory($_POST['time_spent'],$_POST['job'],$_POST['opis'],$thisstaff->getId(),$_POST['time']))) {
                $msg = sprintf(__('Čas spremenjen za %s ure'),$_POST['time_spent']);
			...
			elseif(!$errors['err']) {
                $errors['err']=__('Ni bilo možno objaviti časa');
            }
			break;
		case 'change_time':			
			foreach($_POST as $id => $value){
				if(is_numeric($id)){
					$response = $ticket->changeTime($value[0], $value[1], $id);
...
/*-------Doda dodaten zavihek za objavo aktivnosti------------*/
$nav->addSubMenu(array('desc'=>__('Potrjeno s strani stranke'),
                           'title'=> __('Potrjeni zahtevki'),
                           'href'=>'tickets.php?a=potrjeni',
                           'iconclass'=>'closedTickets',
						   'id' => 'new-ticket'),
                        ($_REQUEST['a']=='potrjeni'));

$nav->addSubMenu(array('desc'=>__('Aktivnosti'),
                           'title'=> __('Dodaj aktivnost'),
                           'href'=>'tickets.php?a=aktivnost',
                           'iconclass'=>'newTicket',
                           'id' => 'new-ticket'),
                        ($_REQUEST['a']=='aktivnost'));
...
if($ticket) {
	...
	/*----moje----*/
	else if($_REQUEST['a']=='aktivnost')
		$inc = 'ticket-aktivnost.php';
	else if($_REQUEST['a']=='potrjeni')
		$inc = 'ticket-potrjeni.php';
