...
class Ticket {
	...
	function addTimeCategory($time,$cat,$str,$oseba,$dejansko){
		$sql = 'INSERT INTO '.TICKET_TIME_TABLE.'(ticket_id,time_spent,vrsta_dela_id,opis,staff_id,created,time) '
				.'values(' .db_input($this->getId()). ', ' .db_input($time). '+' .db_input($currentTime). ',' .db_input($cat). ','.db_input($str).','.db_input($oseba).',NOW(),'.db_input($dejansko).')';
		return (db_query($sql) && db_affected_rows())?true:false;
	}
	
	function changeTime($time,$time_spent,$id){
		$sql = 'UPDATE '.TICKET_TIME_TABLE.' SET time = '.$time.', time_spent = '.$time_spent.' WHERE id = '.$id.'';
		return (db_query($sql) && db_affected_rows())?true:false;
	}	
	/* <<code>> */
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function delete($comments='') {
		...
		$sql2 = 'DELETE FROM '.TICKET_TIME_TABLE.' WHERE ticket_id=' .$this->getId();
