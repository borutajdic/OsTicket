...
class Ticket {
	...
	/*------Doda cas in vrsto delo v bazo--------*/
	var $ticketTime; //Ticket time obj
	function addTimeCategory($time,$cat,$str,$oseba,$dejansko){
		$sql = 'INSERT INTO ost_ticket_time(ticket_id,time_spent,vrsta_dela_id,opis,staff_id,created,time) values(' .db_input($this->getId()). ', ' .db_input($time). '+' .db_input($currentTime). ',' .db_input($cat). ','.db_input($str).','.db_input($oseba).',NOW(),'.db_input($dejansko).')';
		return (db_query($sql) && db_affected_rows())?true:false;
	}
	
	function changeTime($time,$time_spent,$id){
		$sql = 'UPDATE ost_ticket_time SET time = '.$time.', time_spent = '.$time_spent.' WHERE id = '.$id.'';
		return (db_query($sql) && db_affected_rows())?true:false;
	}
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function delete($comments='') {
		...
		*/
		...
		$sql2 = 'DELETE FROM ost_ticket_time WHERE ticket_id=' .$this->getId();
		if(!db_query($sql) || !db_query($sql1) || !db_affected_rows() || !db_query($sql2))
			return false;
