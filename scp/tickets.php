...
if($_POST && !$errors):
    ...
    if($ticket && $ticket->getId()) {
		...
		case 'add_time':
			if(!$_POST['time_spent'])
				$errors['time_spent']=__('Actual time spent is required');
			if(!$_POST['time'])
				$errors['time']=__('Time spent is required');
			...
			elseif(!$errors['err']) {
                $errors['err']=__('Work cannot be saved');
            }
			break;
		case 'change_time':			
...
$nav->addSubMenu(array('desc'=>__('Customer confirmed tickets'),
                           'title'=> __('Confirmed Tickets'),
                           'href'=>'tickets.php?a=potrjeni',
                           'iconclass'=>'closedTickets',
						   'id' => 'new-ticket'),
                        ($_REQUEST['a']=='potrjeni'));

$nav->addSubMenu(array('desc'=>__('Activities'),
