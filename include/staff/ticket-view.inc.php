...
<ul id="threads">
	...
	<!-- <<code>> -->
	<li><a  id="toggle_time_thread" href="#change_time" style="display: none;"><?php echo __('Ticked Time'); ?></a></li>
	<!-- <<code>> -->
</ul>
...
<!-- <<code>> -->
<!-- zavihek ki prikazuje cas moje-->
<div id = "ticket_time" style="display: none;">
<form id="change_time" action="tickets.php?id=<?php echo $ticket->getId(); ?>#change_time" name="change_time" method="post" enctype="multipart/form-data">
<?php csrf_token(); ?>
<input type="hidden" name="ticket_id" value="<?php echo $ticket->getId(); ?>">
<input type="hidden" name="a" value="change_time">
	<?php 
	$getTicket = $ticket->getId();
	$sql = "SELECT tt.time_spent, tt.opis AS opis, d.opis AS delo, CONCAT_WS(' ',s.firstname,s.lastname) as name, date_format(tt.created, '%d-%m-%Y') AS created, tt.id, tt.time
			FROM ost_ticket t
			...
			WHERE t.ticket_id = $getTicket
			ORDER BY created
			";
	if(!$result = db_query($sql)) die("Neki je narobe");
	//die(db_num_rows($result));
	echo '<br>';
	echo '<table style="border-collapse: collapse;width: 100%; border:solid black 1px;  background-color:#F4FAFF;">';
	echo '<tr>
			<th style="text-align:left;">Ustvarjen</th>
			<th style="text-align:left;">Ime</th><th>Opis</th>
			<th style="text-align:left;">Delo</th>
			<th style="text-align:left;">Dejansko delo</th>
			<th style="text-align:left;">Obračun delo</th></tr>';
	while($row = db_fetch_array($result)){
		if($row['id'] != NULL){
			echo '<tr>
				...
				<td>'.$row['opis'].'</td>
				<td>'.$row['delo'].'</td>
				<td><input type="text" value="'.$row['time'].'" name="'.$row['id'].'[]"></td>
				<td><input type="text" value="'.$row['time_spent'].'" name="'.$row['id'].'[]"</td>
				</tr>
				';
		}
	}
	...
	<br>
	<br>
	<input class="btn_sm" type="submit" value="<?php echo __('Spremeni čas');?>">
</form>
</div>
<!-- <<code>> -->
...
<div id="response_options">
    <ul class="tabs">
		...
		<?php
        if($thisstaff->canPostReply()) { ?>
		<li><a id="time_tab" href="#add_time"><?php echo __('Add time');?></a></li>
		<?php
        } ?>
		<!-- <<code>> -->
    </ul>
...
?>
	...
	<form id="add_time" action="tickets.php?id=<?php echo $ticket->getId(); ?>#add_time" name="add_time" method="post" enctype="multipart/form-data">
		<?php csrf_token(); ?>
        <input type="hidden" name="ticket_id" value="<?php echo $ticket->getId(); ?>">
        <input type="hidden" name="a" value="add_time">
		<table style="width:100%" border="0" cellspacing="0" cellpadding="3">
			<tr>
                <td>
                    <label for="time"><b>Dejanski čas dela:</b></label>
                    <input type="text" name="time" class="time" size="5"
                    value="<?php if(isset($_POST['time'])) echo $_POST['time'];?>" />
                    <span class="time" for="time">(0.75 = 45 minut)</span>
                    <br />
                </td>
				<td>
					<label for="time_spent"><b>Obračunski čas</b><label>
					<input type="text" name="time_spent" class="time" size="5" value="<?php if(isset($_POST['time_spent'])) echo $_POST['time_spent'];?>"/>
					<span class="time" for="time_spent">(0.75 = 45 minut)</span>
				</td>
            </tr>
			<tr>
				<td>
					<label for="time_category"><b>Vrsta dela: </b></label>
					<select name="job">
						<?php
						$dbquery='SELECT opis,id FROM '.VRSTA_DELA_TABLE;
			...
			<tr>
				<td>
					<label for="time_spent"><b>Opis dela(opcijsko) </b></label>
					<textarea name="opis" value="" rows="8" cols="50" maxlength="64"></textarea>
				</td>
			</tr>
        </table>
       <p  style="padding-left:165px;">
           <input class="btn_sm" type="submit" value="<?php echo __('Dodaj čas');?>">
       </p>
	</form>
...
<script type="text/javascript">
$(function() {
	$(document).on('change', 'input.time', function(e) {
		...
		if (isNumber(val))
		{		
			var minute = val * 60; //minute
			var m = minute % 60;
			var hou =  Math.floor(minute / 60) % 24;
			var day = Math.floor(minute / (24 *60));
						
			res = '('+val+' h = ' + ( day > 0 ? day + ' dni ' : '')
									+ ( hou>0 ? hou + ' h ' : '') 
									+ (m > 0 ? m +' minut' : '');	
		}
		
		$("span.time[for='"+name+"']").text(res);
	});
